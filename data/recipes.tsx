// Array of recipe objects
export const recipes = [
    {
      id: '1',
      name: "Chicken Fajitas with Guacamole",
      readyAt: 15,
      difficulty: "easy",
      image: require('../assets/images/chicken-fajitas-with-guacamole.jpg'),
      ingredients: 
`For the Chicken:
500g Chicken Breast Fillet
1 yellow bell pepper
1 red bell pepper
1 onion
2 tbsp olive oil
1 tsp garlic powder
1 tsp red chili flakes
1/2 tsp ground cumin
Salt, to taste
Pepper, to taste
For the Guacamole:
1 avocado
1/2 bunch fresh cilantro
1/2 clove garlic
1/2 white onion, finely diced
1 tomato (or 10 cherry tomatoes), finely chopped
Juice of 1/2 lime
Salt, to taste
Red pepper (optional)
For Serving:
6 small tortillas
200g thick Greek yogurt (10%) in a bowl
3 limes, quartered
Fresh cilantro, finely chopped
300g shredded cheddar cheese`,
      preparationSteps: 
`For the Chicken:
Slice the bell peppers and onion very thinly.
Similarly, cut the chicken into very thin strips.
Mix all the ingredients well.
Cook in the air fryer at 200°C for 6 minutes or until completely cooked.
For the Guacamole:
Cut the avocado in half, remove the pit, and scoop out the flesh with a spoon.
Place the flesh in a bowl and mash with a fork. Do not use a blender as it will ruin the texture.
Add all the remaining ingredients and mix.
For Serving:
Serve all the ingredients on a large platter and fill each tortilla with the ingredients of your choice.`,
      productName: "Chicken Breast Fillet",
      productImage: require('../assets/images/products/chicken-breast-fillet.png'), 
      wayOfCooking: "Airfryer",
      tip: ""
    },
    {
      id: '2',
      name: "Stir Fry with Chicken",
      readyAt: 20,
      difficulty: "easy",
      image: require('../assets/images/stir-fry-with-chicken.jpg'),
      ingredients: 
`For the marinade:
400 g Chicken Leg Fillet
3 tbsp soy sauce
1 tbsp brown sugar
1 tbsp corn flour
1 tbsp mustard with honey

For the sauce:
1⁄2 cup chicken stock
3 tbsp oyster sauce
1 tbsp fish sauce
1 tbsp brown sugar
2 peppers, chopped

For the stir-fry:
1 small head of broccoli cut into strips
1 red pepper cut into sticks
1 small onion, finely chopped
2 cloves of garlic, chopped
1 piece of fresh ginger chopped into sticks
3 tbsp wine
1 tbsp cornflour dissolved in a little water
3 tbsp Asian sesame oil
a little oil for frying`,
      preparationSteps: 
`Mix the broth, oyster sauce, fish sauce, brown sugar, chopped peppers, 
and cornstarch dissolved in water in a bowl and set it aside. 
Cut the meat into small rectangular pieces and place them in a bowl. 
For the marinade, combine soy sauce, brown sugar, cornstarch, and honey mustard, 
mixing until the brown sugar dissolves. 
Add the chicken to the marinade and mix well, then cover with plastic wrap and let it rest for 30 minutes.
Heat a little oil in a wok or deep frying pan, then add the onion, garlic, and ginger, stirring for 1 minute. 
Add the meat with its marinade and cook for 4-5 minutes. 
Then, add the wine, broccoli, and bell pepper. 
Serve with jasmine or basmati rice and drizzle with Asian sesame oil.`,
      productName: "Chicken Thigh Fillet",
      productImage: require('../assets/images/products/chicken-thigh-fillet.png'), 
      wayOfCooking: "Ethnic Cuisines",
      tip: ""
    },
    {
      id: '3',
      name: "Stuffed Chicken with Manouri and and Gruyère Cheese",
      readyAt: 60,
      difficulty: "difficult",
      image: require('../assets/images/stuffed-chicken-with-manouri-and-gruyere-cheese.jpg'),
      ingredients: 
`1 pack of Chicken Breast Fillet
4 slices of prosciutto
100 g cream cheese
100 g grated gruyere
12 cherry tomatoes
2 tbsp dry basil
1 tbsp mustard
4 tbsp olive oil
½ cup white wine
salt
pepper
olive oil for frying`,
      preparationSteps: 
`Cut the chicken fillets lengthwise from the side without separating them. 
Open each fillet to create a larger piece, and season both sides with salt and pepper. 
Mash the manouri cheese with a fork and mix it with the graviera cheese and 1 teaspoon of basil. 
Divide the mixture among the fillets, fold them in half, and wrap each fillet with a slice of prosciutto. 
Place them in a baking dish along with the halved cherry tomatoes.
Mix the olive oil with the mustard and basil. 
Drizzle this mixture over the chicken and tomatoes. 
Cover the dish with aluminum foil and bake in a preheated oven at 200°C (392°F) for 20 minutes. 
Remove the foil and bake for another 5-6 minutes.
Slice the chicken and serve it with the roasted cherry tomatoes and the sauce from the baking dish.`,
      productName: "Chicken Breast Fillet",
      productImage: require('../assets/images/products/chicken-breast-fillet.png'), 
      wayOfCooking: "In the oven",
      tip: ""
    },
    {
      id: '4',
      name: "Chicken Ragu",
      readyAt: 55,
      difficulty: "easy",
      image: require('../assets/images/chicken-ragu.jpg'),
      ingredients: 
`1 package Chicken Legs
6 tbsp olive oil
1 large carrot, finely chopped
1 finely chopped onion
2 cloves of garlic
2 stalks of celery, chopped
1 sprig of rosemary
2 sprigs of fresh thyme
1 bay leaf
1⁄2 cup red wine
2 packages of whole tomatoes or chopped tomatoes
salt
pepper
boiled pasta
parmesan or kefalotyri cheese`,
      preparationSteps: 
`In a pot, heat half of the olive oil and brown the chicken pieces evenly on all sides for about 5-6 minutes.
At the same time, in a frying pan, heat the remaining olive oil and sauté the chopped vegetables with thyme and rosemary for 5-6 minutes, 
stirring regularly to prevent them from coloring.
Deglaze the chicken with wine and let the alcohol evaporate for 1-2 minutes. 
Then add the sautéed vegetables, tomatoes, and a little water.
Add salt and pepper, cover the pot, and let the ragu simmer for about 45 minutes, until the chicken is tender. 
Remove the chicken pieces and let them cool slightly.
Shred the meat and chop it finely with a knife or by hand.
Using a blender, puree the sauce with the vegetables after removing the herbs. 
Taste and adjust the salt and pepper as needed.
Return the chopped chicken to the sauce and simmer on low heat for 5 minutes.
Cook the pappardelle (or any pasta of your choice) in salted water. Drain and drizzle with a little olive oil.
Place the pappardelle in a large shallow dish, sprinkle with Parmesan cheese, and add the ragu along the top.`,
      productName: "Chicken Legs",
      productImage: require('../assets/images/products/chicken-legs.png'), 
      wayOfCooking: "In the pot",
      tip: ""
    },
    {
      id: '5',
      name: "Chicken Drumsticks with Honey",
      readyAt: 30,
      difficulty: "medium",
      image: require('../assets/images/chicken-drumsticks-with-honey.jpg'),
      ingredients: 
`2 packages of Chicken Drumsticks
3 tbsp honey
2 tbsp mustard
2 tbsp balsamic
½ cup black bay leaves
some sweet paprika
1 tbsp cumin
1 tbsp coriander
2 tbsp olive oil
salt`,
      preparationSteps: 
`Prepare the Marinade: In a bowl, combine honey, mustard, balsamic vinegar, blackcurrant liqueur, 
paprika, cumin, coriander, olive oil, and a pinch of salt. Mix well until all the ingredients are combined.
Marinate the Chicken: Place the chicken thighs in a bowl and pour the marinade over them. 
Cover and refrigerate for 2-3 hours to allow the flavors to meld.
Preheat the Oven: Preheat your oven to 200ºC (392ºF).
Prepare for Baking: Remove the chicken thighs from the marinade, reserving the marinade for later. 
Place the chicken thighs in a non-stick baking dish.
Bake the Chicken: Bake in the preheated oven for 20 minutes.
Flip and Glaze: After 20 minutes, flip the chicken thighs to the other side, 
brush them with the reserved marinade, and bake for an additional 15 minutes. 
During this time, brush the chicken 1-2 times with the leftover marinade to achieve a beautiful golden-red color.
Serve: Once cooked, remove the chicken from the oven and let it rest for a few minutes before serving.`,
      productName: "Chicken Drumsticks",
      productImage: require('../assets/images/products/chicken-drumsticks.png'), 
      wayOfCooking: "In the oven",
      tip: "Serve with rice, potato purée, or potato salad."
    },
    {
      id: '6',
      name: "Chicken Drumsticks with Noodles",
      readyAt: 70,
      difficulty: "easy",
      image: require('../assets/images/chicken-drumsticks-with-noodles.jpg'),
      ingredients: 
`7 or 8 Chicken Drumsticks
2 large onions, finely chopped
1 clove of garlic, chopped
½ glass of red wine
1 carrot chopped
½ bunch celery, chopped
1 kilo of tomatoes, grated
1 bay leaf
1 tsp. tomato paste
2 spices
1 nutmeg
2 cinnamon sticks
salt and pepper
olive oil
1 kilo of noodles
150 g grated cheese`,
      preparationSteps: 
`Boil the pasta for about 5 minutes, then cool it well, drizzle with a little oil, and set aside.
In a large pot, heat 6-7 tablespoons of olive oil and sauté the chicken on all sides until it is browned, about 7 minutes. 
Add the onions, garlic, carrot, and celery, and let them soften slightly before deglazing with the wine. 
Allow the alcohol to evaporate for 2-3 minutes, then add the tomatoes, tomato paste, and spices.
Let the dish simmer for about an hour until the meat is tender and the sauce has thickened. Season generously with salt and pepper.
Add the pasta to the sauce and cook for an additional 2 minutes. Serve with grated ladotyri cheese.`,
      productName: "Chicken Drumsticks",
      productImage: require('../assets/images/products/chicken-drumsticks.png'), 
      wayOfCooking: "In the pot",
      tip: "Depending on your oven, you can reduce the cooking time to 45-50 minutes to keep the dish juicier."
    },
    {
      id: '7',
      name: "Chicken Drumsticks in Peanut Sauce (Satay)",
      readyAt: 65,
      difficulty: "medium",
      image: require('../assets/images/chicken-drumsticks-in-peanut-sauce-satay.jpg'),
      ingredients: 
`7 or 8 Chicken Drumsticks
2 tbsp fresh ginger, finely chopped
3 tbsp honey
1 lime zest
1 grated garlic
5 tbsp soy sauce
2 tbsp coconut milk
3 tbsp sesame oil
For the sauce:
1 cup peanut butter
1 cup coconut milk
2 tbsp soy sauce
1 tbsp brown sugar in a little warm water`,
      preparationSteps: 
`Mix all the ingredients in a bowl and let them marinate for about 1 hour. 
Preheat the oven grill to 180ºC and grill the chicken for about 30 minutes.
In another bowl, mix all the sauce ingredients very well until combined.
Serve the grilled chicken alongside the peanut sauce.`,
      productName: "Chicken Drumsticks",
      productImage: require('../assets/images/products/chicken-drumsticks.png'), 
      wayOfCooking: "In the oven",
      tip: ""
    },
    {
      id: '8',
      name: "Exotic Chicken Fillet with Coconut Milk, Sun-Dried Tomatoes & Spinach",
      readyAt: 40,
      difficulty: "easy",
      image: require('../assets/images/exotic-chicken-fillet-with-coconut-milk-sun-dried-tomatoes-and-spinach.jpg'),
      ingredients: 
`1 package of Chicken Thigh Fillet
2 tbsp butter
3-4 chopped cloves of garlic
1-2 teacup baby spinach
200 ml coconut milk
½ cup of chopped sun-dried tomatoes
½ teaspoon grated parmesan cheese
salt
pepper`,
      preparationSteps: 
`Season the fillets with salt and pepper on both sides and place them in a large, sufficiently hot non-stick pan. 
Cook for 4-5 minutes on each side until they turn a rosy color.
Remove the fillets from the pan and set them aside.
Add the butter and garlic to the pan. Lower the heat slightly and sauté until the garlic starts to brown.
Add the sun-dried tomatoes and spinach and sauté everything together for a few minutes.
Return the fillets to the pan, pour in the coconut milk, and add the Parmesan cheese. 
Lower the heat and stir the pan occasionally until the milk thickens.`,
      productName: "Chicken Thigh Fillet",
      productImage: require('../assets/images/products/chicken-thigh-fillet.png'), 
      wayOfCooking: "In the frying pan",
      tip: "You can enjoy it with rice or baked potatoes."
    },
    {
      id: '9',
      name: "Pappardelle with Chicken Ragout",
      readyAt: 55,
      difficulty: "easy",
      image: require('../assets/images/pappardelle-with-chicken-ragout.jpg'),
      ingredients: 
`650 g Chicken Thigh Fillet
2 dry onions
3 carrots
2 stalks of celery
3 tbsp fresh thyme
1 glass of semi-sweet white wine
600 g pappardelle
olive oil
salt
pepper`,
      preparationSteps: 
`Cut each chicken thigh into three pieces, and then into small chunks. 
Heat the frying pan and add oil. Finely chop two onions, three carrots, and two stalks of celery. 
Sauté the chicken until it gets a nice color, then add the vegetables, three teaspoons of tomato paste, and mix everything together.
Add a little sweet white wine, water, fresh thyme, and lower the heat.
Fifteen minutes before the ragout is finished cooking, add some water from the pasta, Parmesan cheese, olive oil, and mix well.`,
      productName: "Chicken Thigh Fillet",
      productImage: require('../assets/images/products/chicken-thigh-fillet.png'), 
      wayOfCooking: "In the pot",
      tip: ""
    },
    {
      id: '10',
      name: "Spicy Schnitzel with Jasmine Rice and Curry Sauce",
      readyAt: 50,
      difficulty: "medium",
      image: require('../assets/images/spicy-schnitzel-with-jasmine-rice-and-curry-sauce.jpg'),
      ingredients: 
`Spicy Chicken Schnitzel Limited Edition
500 g jasmine rice
250 g Chinese cabbage or (ice berg)
5 g curry
50 g dry onion
1 clove of garlic mashed
salt
pepper
250 g milk cream`,
      preparationSteps: 
`Preheat the oven to 200ºC (392ºF). Place the schnitzels on parchment paper and bake for 10 minutes or fry them in a deep pan for 3 minutes. 
First, finely chop the Chinese cabbage; if you can't find it in the market, you can finely chop iceberg lettuce.
For the Jasmine rice: In a medium pot, add water and boil the rice for 15-20 minutes. 
Drain it, let it cool, and then reheat it in the same pot with sesame oil, salt, and pepper. 
Next, let's make the curry sauce. In a container, add olive oil and sauté chopped onion and garlic, 
then deglaze with cream, glazing for 5 minutes before finally adding the curry.
Serve by placing curry sauce at the bottom of the plate and topping it with the spicy schnitzel, sliced, along with the garnishes.`,
      productName: "Spicy Chicken Schnitzel",
      productImage: require('../assets/images/products/spicy-chicken-schnitzel.png'), 
      wayOfCooking: "Ethnic Cuisines",
      tip: "We can use cumin instead of curry."
    }
  ];